...
class PlaceTab extends StatefulWidget {
  ...
  static const androidIcon = Icon(Icons.location_on_rounded);
  static const iosIcon = Icon(CupertinoIcons.music_note);

  const PlaceTab ({Key? key, this.androidDrawer}) : super(key: key);
...
class _PlaceTabState extends State<PlaceTab> {
  ...
  Widget _listBuilder(BuildContext context, int index) {
    ...
    return SafeArea(
      ...
      child: Hero(
        tag: index,
        child: HeroAnimatingPlaceCard(
          place: songNames[index],
          color: color,
          heroAnimation: const AlwaysStoppedAnimation(0),
          onPressed: () => Navigator.of(context).push<void>(
            MaterialPageRoute(
              builder: (context) => PlaceDetailTab(
                id: index,
                place: songNames[index],
                color: color,
              ),
  ...
  Widget _buildAndroid(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(PlaceTab.title),
        actions: [
